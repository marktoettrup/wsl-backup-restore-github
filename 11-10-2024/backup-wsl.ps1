...
$logfile = "$env:userprofile\ONEDRI~1\wsl-scripts\"+[Environment]::MachineName+"-backup-wsl.log"
...
function Backup-WSL {
  ...
  foreach ($distro in $distros -ne '') {
    ...
    New-BurntToastNotification -AppLogo $env:userprofile\ONEDRI~1\wsl-scripts\tux.ico -Text "starting backup of WSL named: $distro"
  ...
  if ($allSuccessful) {
    ...
    # Check if there are more than three child directories
    if ($childDirectories.Count -gt 3) {
    ...
    New-BurntToastNotification -AppLogo $env:userprofile\ONEDRI~1\wsl-scripts\tux.ico -Text "WSL backup completed successfully"
    ...
    New-BurntToastNotification -AppLogo $env:userprofile\ONEDRI~1\wsl-scripts\tux.ico -Text "WSL backup error - please check the log at $logfile"
  }
}
...
if ($wsl_status) {
  ...
  $msgBoxInput = [System.Windows.Forms.MessageBox]::Show('You have ' + $wslCount + ' WSL system(s) installed on this Windows host (' + [Environment]::MachineName + ')' + [Environment]::NewLine + 'Your WSL system(s): ' + $commaSeparatedWSLNames + [Environment]::NewLine + [Environment]::NewLine + 'Click Yes to perform backup of your WSL system(s) now' + [Environment]::NewLine + 'Your WSL system(s) will be closed during the process and backups will be placed in the wsl-backup folder in your One-Drive ' + [Environment]::NewLine + [Environment]::NewLine   + 'Click No, if you want to skip this backup. WSL backup will be attempted again later'  + [Environment]::NewLine + 'According to the trigger of the Scheduled Task named: ' + $TaskName + " " + $taskTrigger + [Environment]::NewLine + [Environment]::NewLine + 'Click Cancel if you want to disable this WSL backup schedule permanently. No further WSL backup attempts will be made','ITM WSL Backup utility','YesNoCancel','Information','Button1','ServiceNotification')
    ...
    switch ($msgBoxInput) {
        ...
        'Yes' {
          New-BurntToastNotification -AppLogo $env:userprofile\ONEDRI~1\wsl-scripts\tux.ico -Text "WSL backup starting"
          Main -Command $args[0] -BackupDirectory $args[1]
        }
        'No' {
          New-BurntToastNotification -AppLogo $env:userprofile\ONEDRI~1\wsl-scripts\tux.ico -Text "The WSL backup process has been closed"
        }
        'Cancel' {
          if (Get-ScheduledTask -TaskName $TaskName -ErrorAction SilentlyContinue) {
            # If the task exists, disable it
            Start-Process pwsh -Verb RunAs -ArgumentList "-NoExit -File $env:userprofile\ONEDRI~1\wsl-scripts\backup-wsl-schedule-disable.ps1"
            New-BurntToastNotification -AppLogo $env:userprofile\ONEDRI~1\wsl-scripts\tux.ico -Text "The WSL backup schedule has been permanently disabled. Run setup.exe if you want to enable it again"
        }
        }
